...
pub fn build_router(state: ApiState) -> Router {
    ...
    let admin_routes = Router::new()
        ...
        .route("/jobs/{job_id}/attempts", get(admin::list_job_attempts))
        .route("/jobs/{job_id}/cancel", post(admin::cancel_job))
        .route("/jobs/{job_id}/retry", post(admin::retry_job))
        .route("/diagnostics/queue", get(admin::diagnostics_queue))
        .route("/diagnostics/storage", get(admin::diagnostics_storage))
        .route(
            "/diagnostics/meili/bootstrap",
            post(admin::start_meili_bootstrap),
        )
        .route(
            "/diagnostics/meili/bootstrap/runs",
            get(admin::list_meili_bootstrap_runs),
        )
        .route(
            "/diagnostics/meili/bootstrap/{run_id}",
            get(admin::get_meili_bootstrap_run),
        )
        .route(
